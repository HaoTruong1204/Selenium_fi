...
from modules.config import DEFAULT_THEME, THEMES, BRAVE_OPTIONS
...
class AutomationView(QWidget):
    ...
    def start_automation(self):
        ...
        # Lấy cấu hình Brave từ settings
        ...
        if not brave_path or not os.path.exists(brave_path):
            self.log_message("❌ Không tìm thấy Brave browser. Vui lòng kiểm tra lại cài đặt.", "error")
            return
            
        # Cấu hình Chrome cho worker
        chrome_config = {
            "chrome_path": brave_path,
            "profile_path": brave_profile,
            "brave_specific_args": BRAVE_OPTIONS.get("brave_specific_args", [])
        }
        ...
        if current_tab == 0:  # Google tab
            keyword = self.google_keyword.text().strip()
            if not keyword:
                self.log_message("❌ Vui lòng nhập từ khóa tìm kiếm", "error")
                return
            ...
            if not email or not password:
                self.log_message("❌ Vui lòng nhập email và mật khẩu", "error")
                return
            ...
            self.worker = EnhancedAutomationWorker(
                ...
                password=password,
                proxy=proxy,
                headless=False,  # Facebook luôn cần visible mode
                chrome_config=chrome_config
            )
            self.results_table.setColumnCount(2)
            self.results_table.setHorizontalHeaderLabels(["Thông tin", "Giá trị"])
            
        elif current_tab == 2:  # Shopee tab
            keyword = self.sp_keyword.text().strip()
            if not keyword:
                self.log_message("❌ Vui lòng nhập từ khóa tìm kiếm", "error")
                return
    ...
    def apply_theme(self):
        """Áp dụng theme cho automation view"""
        try:
            ...
            print(f"Lỗi khi áp dụng theme cho AutomationView: {e}")
            traceback.print_exc()
